...
class EVSimulation:
    ...
    def initialize_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            agent.shop_end_time = (shop_start + shop_duration) % 1440
            ...
            agent.restaurant_end_time = (rest_start + rest_duration) % 1440
    ...
    def run(self) -> List[Dict]:
        ...
        while current_time <= self.SIMULATION_END:
            ...
            for agent in self.agents:
                ...
                # ...existing code for triggers...
                if agent.Day is not None and minute_of_day == (agent.shop_start_time or -1) and day_of_week == agent.Day:
                    if agent.current_state == AgentState.AT_HOME:
                        agent.current_state = AgentState.MOVING_TO_SHOP
                        agent.current_target = agent.shop
                        agent.current_path, _ = self.road_network.dijkstra(agent.current_position, agent.current_target)
                        agent.path_index = 0
                if agent.Day is not None and minute_of_day == (agent.shop_end_time or -1) and day_of_week == agent.Day:
                    if agent.current_state == AgentState.AT_SHOP:
                        agent.previous_target = agent.current_target
                        agent.current_state = AgentState.MOVING_TO_RESTAURANT
                        agent.current_target = agent.restaurant
                        agent.current_path, _ = self.road_network.dijkstra(agent.current_position, agent.current_target)
                        agent.path_index = 0
                if agent.Day is not None and minute_of_day == (agent.restaurant_end_time or -1) and day_of_week == agent.Day:
                    if agent.current_state == AgentState.AT_RESTAURANT:
                        agent.previous_target = agent.current_target
                        agent.current_state = AgentState.MOVING_TO_HOME
                        agent.current_target = agent.home_location
                        agent.current_path, _ = self.road_network.dijkstra(agent.current_position, agent.current_target)
                        agent.path_index = 0
                if minute_of_day == office_start_min:
                    if agent.current_state == AgentState.AT_HOME:
                        if agent.Day is not None and day_of_week == agent.Day:
                            if minute_of_day != (agent.shop_start_time or -1):
                                agent.current_state = AgentState.MOVING_TO_SHOP
                                agent.current_target = agent.shop
                                agent.current_path, _ = self.road_network.dijkstra(agent.current_position, agent.current_target)
                                agent.path_index = 0
                            ...
                            agent.current_path, _ = self.road_network.dijkstra(agent.current_position, agent.current_target)
                            agent.path_index = 0
                if minute_of_day == office_end_min:
