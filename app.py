...
class EVSimulation:
    def __init__(self, roads_file: str, charging_stations_file: str, num_agents: int = 3,
        ...
        # 'random' (randomly pick any station that can accept the agent).
        # To change the allocation behavior, pass allocation_strategy to EVSimulation(),
        # or modify self.allocation_strategy here.
        self.allocation_strategy = allocation_strategy
        # Weight used by the queue+distance strategy to scale queue length influence.
        # Larger value biases toward shorter queues more strongly.
        self.queue_distance_queue_weight = 0.5
        ...
        self.SIMULATION_END = 2880  # 48 hours
    ...
    def initialize_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            energy_consumption_rate = random.uniform(0.35, 0.45)  # kWh/km
    ...
    def find_nearest_charging_station(self, current_position: int) -> Optional[ChargingStation]:
        """Find nearest charging station that can accept the agent"""
    ...
    def find_station_by_queue_and_distance(self, current_position: int) -> Optional[ChargingStation]:
        """Choose a station balancing distance and queue length.

        For each station that can accept the agent, compute a score and choose the station
        with the lowest score. Score here is: distance + (queue_length * queue_weight).
        Distance is the shortest-path distance (km) computed by Dijkstra. The queue weight
        is configurable via self.queue_distance_queue_weight.
        """
        best_score = float('inf')
        best_station = None

        for station in self.charging_stations:
            if not station.can_accept_agent():
                continue

            path, distance = self.road_network.dijkstra(current_position, station.location)
            queue_len = len(station.waiting_queue)
            # Score combines distance (km) and scaled queue length
            score = distance + (queue_len * self.queue_distance_queue_weight)
    ...
    def find_random_charging_station(self, current_position: int) -> Optional[ChargingStation]:
        """Randomly choose any charging station that can accept the agent.

        This ignores distance and queue length and picks uniformly among stations
        that can accept an agent (i.e., whose queue is not full).
        """
        candidates = [s for s in self.charging_stations if s.can_accept_agent()]
        if not candidates:
            return None
        return random.choice(candidates)

    def select_charging_station(self, current_position: int) -> Optional[ChargingStation]:
        """Dispatch to the configured allocation strategy and return a station or None."""
        if self.allocation_strategy == 'queue_distance':
            return self.find_station_by_queue_and_distance(current_position)
    ...
    def run(self) -> List[Dict]:
        ...
        print(f"Starting simulation from {self.SIMULATION_START} to {self.SIMULATION_END}...")
        
        while current_time <= self.SIMULATION_END:
            if current_time % 60 == 0:  # Print every hour
            ...
            for agent in self.agents:
                # --- DEPARTURE TRIGGERS ---
                if current_time == (self.BASE_OFFICE_START_TIME + agent.morning_offset):
                    if agent.current_state == AgentState.AT_HOME:
                        agent.current_state = AgentState.MOVING_TO_OFFICE
                        agent.current_target = agent.office_location
                        agent.current_path, _ = self.road_network.dijkstra(
                            agent.current_position, agent.current_target)
                        agent.path_index = 0
                
                if current_time == (self.BASE_OFFICE_END_TIME + agent.evening_offset):
                    if agent.current_state == AgentState.AT_OFFICE:
                        agent.current_state = AgentState.MOVING_TO_HOME
                        agent.current_target = agent.home_location
                        agent.current_path, _ = self.road_network.dijkstra(
                            agent.current_position, agent.current_target)
                        agent.path_index = 0
                
                # --- LOW BATTERY CHECK ---
                if agent.current_state in [AgentState.MOVING_TO_OFFICE, 
                                          AgentState.MOVING_TO_HOME, 
                                          AgentState.MOVING_TO_CHARGING]:
                    if agent.needs_charging() and agent.current_state != AgentState.MOVING_TO_CHARGING:
                        agent.previous_target = agent.current_target

                        # Select charging station according to configured strategy.
                        # Available strategies: 'nearest' (default), 'queue_distance', 'random'
                        station = self.select_charging_station(agent.current_position)

                        if station:
                            path, distance_to_station = self.road_network.dijkstra(
                                agent.current_position, station.location)
                    ...
                    if agent.path_index < len(agent.current_path) - 1:
                        ...
                        if agent.current_target == agent.office_location:
                            ...
                            station = next((s for s in self.charging_stations 
                                          if s.location == agent.current_target), None)
            ...
            # ===== CHARGING STATION UPDATE PHASE =====
            for station in self.charging_stations:
                # Release fully charged agents
                ...
                # Charge currently connected agents
                station.update_charging(self.agents, self.TIME_STEP)
                
                # Allocate ports to waiting agents
